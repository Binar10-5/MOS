...
class Order extends Model
{
    ...
    protected $fillable = [
        ...
        , 'coupon_id', 'payment_data', 'language_id', 'transportation_company_id' ,'tracking_number'
    ];
    ...
    {
        if(!empty($min) && !empty($max)){
            $query->whereBetween('subtotal', [$min, $max]);
        }
    }
    ...
    public function scopeCreated($query, $date_star, $date_end)
    {
        if(!empty($date_star) && !empty($date_end)){
            $query->whereBetween('created_at', [$date_star, $date_end]);
        }
    }
}
